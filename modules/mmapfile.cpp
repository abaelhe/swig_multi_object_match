...
uint8_t *MmapPointer::GetPointer() {
    return _pData;
}

bool MmapPointer::Allocate(int fd, uint64_t cursor, uint32_t windowSize, uint64_t size) {
    Free();
    ...
    while (_cursor + _size < cursor + size) {
        LOG_MMAP("We need to go a little bit further; Wanted: %" PRIu64 "; got: %" PRIu64, cursor + size, _cursor + _size);
        _size += MmapFile::_pageSize;
    }

    LOG_MMAP("Reading %" PRIu64 " bytes from disk", _size);
    _pData = (uint8_t *) mmap(NULL, _size, PROT_READ, MAP_FILE | MAP_SHARED, fd, _cursor);
...
bool MmapFile::Initialize(string path, uint32_t windowSize, bool exclusive) {
    ...
    _windowSize = (pagesCount * _pageSize) + (((windowSize % _pageSize) != 0) ? _pageSize : 0);
    _path = path;
    ...
    if (_windowSize == 0){
        _windowSize =( _size / _pageSize) * _pageSize + (((_size % _pageSize) != 0) ? _pageSize : 0);
    }
    //6. We are done
    return true;
}
...
bool MmapFile::PeekBuffer(uint8_t *pDestBuffer, uint64_t count) {
    ...
    if (_windowSize < count) {
        DEBUG("_cursor: %" PRIu64 "; count: %" PRIu64 "; %s", _cursor, count, STR(_path));
        FATAL("Invalid window size: _windowSize < count %u < %" PRIu64, _windowSize, count);
        _failed = true;
        return false;
    }
