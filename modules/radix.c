...
#ifndef LSB_FIRST
// MSB: 数 k1 和 k2 两个bit stream 不同的 bit 数;
// RETURN: 不同的 bit 数目;
static inline int count_bits(char *k1, char *k2, int count)
{
    int mask = 128; // 1字节的最高bit,对应的数值.
    while (~(*k1 ^ *k2) & mask && --count) {// 第 mask 高位 bit 是相同;
        mask >>= 1; // 右进1bit
        if (0 == mask) { // 一字节里面全部的 bit 都相同;
            mask = 128; // 继续比较 当前字节里的 高bit: Most Significant Bit;
            k1 += 1; // k1 右移1字节,继续比较;
            k2 += 1; // k2 右移1字节,继续比较
        }
    }
...
static inline int count_bits(char *k1, char *k2, int count)
{
    int mask = 1; // 1字节的最右bit,对应的数值.
    while (~(*k1 ^ *k2) & mask && --count) { // 判断第 mask 位 bit 相同; 只要index为 mask 的 bit 不同，就停止 while 循环.
        mask <<= 1; //再左进一位;
        if (256 == mask) {// 前8比特比较都已经是相等的.
            mask = 1; // 再比较进1字节的 第一bit
            k1 += 1; // k1 移进一字节;
            k2 += 1; // k2 移进一字节;
        }
    }
    return count; // count: 是相同 bits 最多的数目;
}
#endif

// 数 k1 和 k2 两个 bit stream 最多 max 个相同的 bit;
// RETURN: 相同的 bit 数目, 不超过 max;
static int count_common_bits(char *k1, char *k2, int max)
{
    int count = max;
    // XXX SIMD-ify?
    while (*k1 == *k2 && count >= sizeof(int) * 8) { // 长于 32bit: 4字节, CPU L1&Cache
    ...
    while (*k1 == *k2 && count >= 8) { // 长于 8bit: 1字节
        k1++;
        k2++;
        count -= 8;
    }

    return max - count_bits(k1, k2, count); //count 小于8, 小于 1字节: 8 bit,
}


#ifndef LSB_FIRST
static inline int shift(int i) // shift ;  MSB ：高 bit
{
    return 128 >> (i & 7);
}
#else
static inline int shift(int i) // 相当于取模: i % 8 取得 8 bit 以内的 index ；  LSB ：低 bit
{
    return 1 << (i & 7); // 1<< [0,1,2,3,4,5,6,7] 作为 mask 提起第 i%8 的bit;
     //i&0111: 少于 1 byte (8 bit) 的 比特数;
}
#endif

static inline int get_bit_at(char *k, int i) // 在 k 比特流里，提取 index 为 i 的bit 值;
{
    int bytes = i >> 3; //  相同于 i / 8 = 字节数;  i
    int mask = shift(i); // 相同于 i % 8 =
    k += bytes; // k 移动 bytes 整字节数, (i/8, 也就是 i>>3);
    return *k & mask; // 提取 k[bytes] 里的bits
}
...
static int insert_leaf(rxt_node *newleaf, rxt_node *sibling, rxt_node *parent)
{
    ...
    max_len = rdx_min(newleaf->pos, sibling->pos); // 取
...
// color: 1 for leaf, 0 for inner, 2 for ONLY one branch leaf.
static int insert_internal(rxt_node *newleaf, rxt_node *n)
{
    // FIRST: check for common bits
    rxt_node *left = n->left, *right = n->right;
    int bits   = count_common_bits(newleaf->key, left->key, rdx_min(newleaf->pos, left->pos));
    int bits2  = count_common_bits(newleaf->key, right->key, rdx_min(newleaf->pos, right->pos));
    ...
    if (bits >= bits2) {
        ...
        return insert_internal(newleaf, left); // `color==0` for internal Node
    } else {
        if (right->color){
            return insert_leaf(newleaf, n->right, n);
        }
...
static inline int keylen(char *str) // keylen 区别于 strlen 之处.
{
    int len = strlen(str);
...
#define NEWLEAF(nl, k, v) \
    ...
    if (NULL == n->left && NULL == n->right) {
        ...
        // count bits in common: newleaf 和 sib 相同的 bit 数;
        bits = count_common_bits(key, sib->key, rdx_min(newleaf->pos, sib->pos));

        if (get_bit_at(key, bits)) { // 右为 1;  index 为 bits 的 bit 为 1
            n->right = newleaf;
            n->left = sib;
        } else { // 第 bits 的 bit 为 0; 左为 0;
            n->right = sib;
            n->left = newleaf;
        }
        n->value = NULL;
...
static void reset_key(char *key, char *newkey, rxt_node *n)
{
    ...
    if (key == n->key) {
        n->key = newkey;
        if (n->left && n->right){
            n->pos = count_common_bits(n->left->key, n->right->key, rdx_min(n->left->pos, n->right->pos));
        }
        if (n->parent)
